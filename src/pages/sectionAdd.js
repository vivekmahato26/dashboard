...
import {getSections,addSections} from "../redux/slices/section"
...
export default function SectionAdd() {
  ...
  const sectionMap = location.state.sections
  ...
  const addTitle = () => {
    ...
    const res = sections.addSections.__typename;

    console.log(res);
    if(res === "res") {
    dispatch(getSections());
    } else {
      console.log(sections.addSections)
    }
    toggleModal(); 
      ...
      <div className={styles.sectionContainer}>
        ...
        <div className={styles.sectionGrid}>
                ...
                <Collapse accordion={true}  expandIconPosition="end">
                  <Panel header={
                    <div>
                      ...
                      {c.Section && c.Section.map((s)=> {
                      return(
                        <>
                        <div className={styles.accodionText}>
                          <p className={styles.sectionText}>{s.title}</p>
                      ...
                      <div className={styles.accodionAddBtnContainer}>                        
                        <button onClick={() => h(c)} className={styles.accodionAddBtn}>Add</button>                                               
                      </div>
                  </Panel>
                </Collapse>
          ...
          <div className={styles.sectionRightGrid}>
            ...
            <div className={modal ? styles.sectionRightContainer : styles.sectionRightContainerNone}>
                    ...
                    <div className={styles.modalForm}>
                      ...
                      <div>
                        <div className={styles.projectTitle}>
                            ...
                            <input type="text" value={inputData.title} onChange= {(event) => setInputData({...inputData,title:event.target.value})} />
                        </div>
                        <div className={styles.projectTitle}>
                            <label className={styles.objectivesLabel} htmlFor="">Description</label>
                            <input type="text" value={inputData.description} onChange= {(event) => setInputData({...inputData,description:event.target.value})}/>
                        </div>
                        ...
                        <div className={styles.projectImageDiv}>
                          ...
                          >
                            ...
                            {creds.clientPayload && (
                              <>
                                <input type="hidden" name="key" value={creds.clientPayload.key} />
                                ...
                                <input
                                  ...
                                  value={creds.clientPayload["x-amz-credential"]}
                                />
                                <input
                                  type="hidden"
                            ...
                            <input ref={fileRef} className={styles.fileImageInput} type="file" onChange={getCreds} />
                        ...
                        <div className={styles.projectImageDiv}>
                            ...
                            <input className={styles.fileImageInput} type="file" accept="image/*" />
                            <button className={styles.uploadButton}>Upload</button>
                        </div>
                        <div className={styles.projectTitle}>
                            <label htmlFor="">Source Code</label>
                            {/* <input className={styles.taskInput} type="textarea" /> */}
                            <textarea name="" className={styles.addTaskTextArea} cols="60" rows="5" value={inputData.type} onChange= {(event) => setInputData({...inputData,type:event.target.value})}></textarea>
                        </div>
                                
                        <div className={styles.projectImageDiv}>
                            <label className={styles.uploadImageLabel} htmlFor=""></label>
                            <input className={styles.fileImageInput} type="file"  value={inputData.code} />
                            <button className={styles.uploadButton} onClick= {(event) => setInputData({...inputData,code:event.target.value})}>Upload</button>
                        </div>
                        <button  onClick={() => {addTitle()}} className={styles.saveButton}>save</button>
                      </div>
